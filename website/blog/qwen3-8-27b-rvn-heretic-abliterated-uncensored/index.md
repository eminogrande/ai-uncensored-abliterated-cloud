...
- RVN adds two extra weight-tweaking passes to trohrbaugh's Qwen3.8 edit and ships only as an easy-to-run file.
- 0bserverx reported 0 to 1 of 100 refusals on a forced-start test; some users still reported refusals.
- RVN's corrupted compressed file was rebuilt and re-uploaded in August 2026.

## Basically, the facts

**Why three passes instead of one**

Basically, 0bserverx reported less answer drift after two extra weight-tweaking passes on the Qwen3.8 edit.

**ARA is a different animal from directional abliteration**

Basically, ARA tweaks the model's numbers directly instead of removing just one refusal pattern.

**The GGUF-only release, and the KV math behind it**

Basically, RVN's easy-to-run files drop the shortcut parts, and the recommended one is 16.55 GB.

**A corrupted quant, caught by the community**

Basically, 0bserverx traced RVN's broken file to bad scaling values and blanked-out internal parts.
...
Basically, the RVN article gives run commands that pick one compressed file explicitly.

**How we treat it**

Basically, RVN's reported 0 to 1 of 100 refusals came from a forced-start test, not every possible prompt.

**The idea, in plain words**

Basically, ARA tweaks a model's internal numbers to cut refusals while keeping harmless answers mostly intact.
<!-- /READING-TLDR -->
<!-- ARTICLE-META-MD -->
