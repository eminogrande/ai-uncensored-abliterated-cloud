...
def test_catalog_loads_unique_models() -> None:
    settings = load_settings()

    assert settings.default_model == "qwen36"
    assert list(settings.models) == [
        "qwen36",
        "ornith35",
        "qwythos9",
        "ornith397",
    ...
    assert len({model.lifecycle_key for model in settings.models.values()}) == 5
    assert {
    ...
    assert settings.models["qwen38u"].deployment_enabled is False
    assert all(
...
def test_modal_app_names_use_model_names_not_role_labels() -> None:
    ...
    assert {
        ...
        "qwen38u": "orcarouter-qwen3-8-27b-uncensored-fp8",
    }
    assert all(
